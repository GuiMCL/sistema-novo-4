...
async function instanciaExcluir(instId) {
  if (!confirm('Excluir esta instância permanentemente? O pareamento será perdido.')) return;
  try {
    const r = await fetch('/admin/instancia/' + instId + '/excluir', { method: 'POST' });
    if (!r.ok) throw new Error('HTTP ' + r.status);
    toast('ok', 'Instância excluída');
    setTimeout(() => location.reload(), 800);
  } catch(e) {
    toast('erro', 'Erro ao excluir instância');
  }
}
...
window.instanciaExcluir = instanciaExcluir;
